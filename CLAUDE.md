...
## 6. 当前状态（v1.0 — Phase 10 完成时）

> 进度：Phase 1–10 全部完成 —— 项目已发布 **v1.0**。顶栏版本号 `v1.0`。
> 10 种测试模式 / 双引擎 / 侧边栏 / 多语言导出 / 匹配详情 / 主题平滑过渡 / 完整快捷键 全部就绪。
...
> 进度标记：Phase 2–10 全部完成 ✅。下面的验收清单保留作为各阶段的规格说明与回归参考。
...
3. **第三步**：项目已发布 v1.0。如果用户提出新需求，参考本文件 §3 的约定。
   后续若要加新功能（更多模式 / 新语言代码生成 / VS Code 扩展导出 等），
   仍按"小步交付 + 每步可演示"的方式推进。
